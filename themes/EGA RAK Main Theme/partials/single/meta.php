...
<ul class="meta ospm-<?php echo $sp_meta_sep_class; ?> clr" style="border: none; margin-bottom: 0px;"> <!-- Assem Ali 14th August -->
	...
	foreach ( $sections as $section ) {
		...
		<?php if ( 'date' === $section && has_post_thumbnail() && ! empty( $categories) && $categories[0]->term_id != 861 && $categories[0]->term_id != 864 ) { // Assem Ali, 15th August 2024, added the thumbnail check to post date for posts with featured_image only ?>
			<li class="meta-date"<?php oceanwp_schema_markup( 'publish_date' ); ?>><span class="screen-reader-text"><?php esc_html_e( 'Post published:', 'oceanwp' ); ?></span><?php //oceanwp_icon( 'date' ); ?><h4 style="margin-bottom: 0px;">
				<?php echo get_the_date(); // Assem Ali 14th August?>
				</h4></li>
		<?php } ?>
