...
class OceanWP_Breadcrumb_Trail {
	...
	protected function set_labels() {
		$lang_check = function_exists('pll_current_language') && pll_current_language() === 'ar';
		$search_translation_text = $lang_check ? 'نتيجة البحث عن' : 'Search results for';
		$page_translation_text = $lang_check ? 'صفحة ' : 'Page ';
		$home_translation_text = $lang_check ? 'الرئيسية' : 'Home';
		$not_found_translation_text = $lang_check ? 'صفحة غير موجودة' : 'Not found 404';
		
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'oceanwp' ),
			'home'                => esc_html__( $home_translation_text, 'oceanwp' ),
			'error_404'           => esc_html__( $not_found_translation_text, 'oceanwp' ),
			'archives'            => esc_html__( 'Archives', 'oceanwp' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( $search_translation_text, 'oceanwp' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( $page_translation_text . '%s', 'oceanwp' ),
